...
def load_hf_lm_and_tokenizer(
    ...
    if gptq_model:
        ...
        if device_map:
            model = AutoModelForCausalLM.from_pretrained(
                model_name_or_path, device_map=device_map, torch_dtype=None
            )
        else:
            model = AutoModelForCausalLM.from_pretrained(
    ...
    **completion_kwargs,
    ...
    for i in range(0, len(instances), batch_size):
        ...
        if retry_count == retry_limit:
            raise RuntimeError(
                f"Failed to get response from OpenAI API after {
                    retry_limit} retries."
            )
