...
argparser.add_argument(
    "--train_file_names", type=str, nargs="+", help="The name of the training file"
)
argparser.add_argument("--ckpts", type=int, nargs="+", help="Checkpoint numbers.")
argparser.add_argument(
    "--checkpoint_weights", type=float, nargs="+", help="checkpoint weights"
)
argparser.add_argument(
    "--target_task_names", type=str, nargs="+", help="The name of the target tasks"
)
argparser.add_argument(
    "--validation_gradient_path",
    type=str,
    default="{} ckpt{}",
    help="The path to the validation gradient file",
)
argparser.add_argument(
    "--output_path", type=str, default="selected_data", help="The path to the output"
)
...
def calculate_influence_score(
    training_info: torch.Tensor, validation_info: torch.Tensor
):
...
for target_task_name in args.target_task_names:
    for train_file_name in args.train_file_names:
        ...
        for i, ckpt in enumerate(args.ckpts):
            ...
            influence_score += args.checkpoint_weights[i] * calculate_influence_score(
