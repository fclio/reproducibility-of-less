...
if __name__ == "__main__":
    ...
    for target_task in args.target_task_names:
        ...
        score_paths = [
            os.path.join(output_path, f"{task_name}_influence_score.pt")
        ...
        if not os.path.exists(sorted_score_file):
            with open(sorted_score_file, "w", encoding="utf-8") as file:
                ...
                for score, index, name in zip(sorted_scores, sorted_index, data_from):
                    file.write(f"{args.train_file_names[name.item()]}, {index.item()}, {round(score.item(), 6)}\n")
        ...
        with open(
            os.path.join(output_path, f"top_{data_amount_name}.jsonl"),
            "w",
            encoding="utf-8",
            errors="ignore",
        ) as file:
