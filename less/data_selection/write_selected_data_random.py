...
def parse_args():
    ...
    argparser.add_argument('--train_file_names', type=str,
                           nargs='+', help='The names of the training score files (without extension)')
    ...
    argparser.add_argument('--target_task_names', type=str,
                           nargs='+', help='The name(s) of the target tasks')
    argparser.add_argument('--output_path', type=str,
                           default="selected_data", help='The path to the output directory')
    argparser.add_argument('--max_samples', type=int,
                           default=None, help='The maximum number of samples to select')
...
if __name__ == "__main__":
    ...
    for target_task in args.target_task_names:
        ...
        score_paths = [os.path.join(
            output_path, f"{task_name}_random_influence_score.pt") for task_name in args.train_file_names]
        ...
        data_from = torch.cat([torch.ones(line_num, dtype=torch.long)*i 
                              for i, line_num in enumerate(num_samples)]).to(device)
        ...
        sorted_score_file = os.path.join(output_path, f"random_sorted.csv")
        if not os.path.exists(sorted_score_file):
            with open(sorted_score_file, 'w', encoding='utf-8') as file:
                ...
                for score_val, idx_val, from_val in zip(sorted_scores, sorted_index, data_from):
                    file.write(
        ...
        print(f"Saved top {args.max_samples} random selected data to {top_file}")
