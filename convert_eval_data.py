...
def load_bm25_results(bm25_path):
    """Load BM25 ranked results from the JSON file."""
    if not os.path.exists(bm25_path):
        raise FileNotFoundError(f"BM25 results file not found: {bm25_path}")
    with open(bm25_path, "r", encoding="utf-8") as f:
        return json.load(f)

def convert_beir_to_pointwise(data_path, bm25_results, num_documents_per_query=100, split="test", dev_mode=False):
    """
    Converts a BEIR dataset into a pointwise format using BM25-ranked documents.
    """
    pointwise_data = []
    prompt = "Given a query A and a passage B, determine whether the passage contains an answer to the query by providing a prediction of either 'Yes' or 'No'."
    ...
    # # only use the first query for now
    # queries = {k: v for k, v in queries.items() if k == list(queries.keys())[0]}

    for query_index, (query_id, query_text) in tqdm(enumerate(queries.items(), start=1), desc=f"Processing queries ({split})"):
        if query_id not in bm25_results:
            print(f"Skipping query {query_id} as no BM25 results found.")
            continue  # Skip queries without BM25 results

        ranked_doc_ids = list(bm25_results[query_id].keys())[:100]  # Get top 100 docs
        relevant_doc_ids = {doc_id for doc_id, score in qrels.get(query_id, {}).items() if score > 0}
        
        positive_doc_ids = [doc_id for doc_id in ranked_doc_ids if doc_id in relevant_doc_ids]
        negative_doc_ids = [doc_id for doc_id in ranked_doc_ids if doc_id not in relevant_doc_ids]

        num_positive = min(len(positive_doc_ids), num_documents_per_query // 2)
        num_negative = num_documents_per_query - num_positive
        query_distribution[(num_positive, num_negative)] += 1

        if dev_mode:
            if positive_doc_ids and negative_doc_ids:
                pointwise_data.append({
                    "dataset": data_path.split("/")[-1],
                    "id": f"{data_path.split('/')[-1]}_dev_{query_id}_{query_index}_pos",
                    "messages": [
                        {"role": "user", "content": f"{prompt} Query: {query_text} Passage: {corpus[positive_doc_ids[0]]['text']}"},
                        {"role": "assistant", "content": "Yes"}
                    ]
                })
                pointwise_data.append({
                    "dataset": data_path.split("/")[-1],
                    "id": f"{data_path.split('/')[-1]}_dev_{query_id}_{query_index}_neg",
                    "messages": [
                        {"role": "user", "content": f"{prompt} Query: {query_text} Passage: {corpus[negative_doc_ids[0]]['text']}"},
                        {"role": "assistant", "content": "No"}
                    ]
                })
            if len(pointwise_data) >= 20:
                break
            continue

        for i in range(min(num_positive, len(positive_doc_ids))):
            pointwise_data.append({
                "dataset": data_path.split("/")[-1],
                "id": f"{data_path.split('/')[-1]}_{query_id}_{query_index}_{i+1}",
                "messages": [
                    {"role": "user", "content": f"{prompt} Query: {query_text} Passage: {corpus[positive_doc_ids[i]]['text']}"},
                    {"role": "assistant", "content": "Yes"}
                ]
            })
        
        for i in range(min(num_negative, len(negative_doc_ids))):
            pointwise_data.append({
                "dataset": data_path.split("/")[-1],
                "id": f"{data_path.split('/')[-1]}_{query_id}_{query_index}_{num_positive+i+1}",
                "messages": [
                    {"role": "user", "content": f"{prompt} Query: {query_text} Passage: {corpus[negative_doc_ids[i]]['text']}"},
                    {"role": "assistant", "content": "No"}
                ]
            })
...
def process_beir_dataset(subdataset, num_documents_per_query=100):
    """
    Processes a BEIR dataset using BM25 rankings and saves the output.
    """
    print(f"\n🚀 Processing dataset: {subdataset}")
    data_path = f"./datasets/{subdataset}"
    output_path = f"./data/eval/{subdataset}"
    bm25_path = f"./bm25_output/results_{subdataset}.json"
    ...
    bm25_results = load_bm25_results(bm25_path)
    
    formatted_data = convert_beir_to_pointwise(data_path, bm25_results, num_documents_per_query, split="test")
    formatted_dev_data = convert_beir_to_pointwise(data_path, bm25_results, 2, split="test", dev_mode=True)
    ...
    print(f"✅ Full dataset saved to {output_file_train}")
    print(f"✅ Dev dataset saved to {output_file_dev}")

# Example usage
datasets = ["nfcorpus", "scidocs", "fiqa"]
for dataset in datasets:
    process_beir_dataset(dataset)